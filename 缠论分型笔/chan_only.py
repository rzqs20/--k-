...
from chan_report import (
    cb, load_bars, resolve_symbol, _parse_date, FREQ_ALIASES, _echarts_src,
)

# =====================================================================
# HTML 模板（纯缠论：K线 + 笔 + 线段 + 分型 + 成交量）
# =====================================================================
...
<div class="wrap">
  ...
  <div class="footer">本报告由 chan_only.py 自动生成（纯缠论：分型/笔/线段，不含布林/RSI/中枢/买卖点）｜ 技术分析仅供参考，不构成投资建议</div>
</div>
<script>
var DATA=__DATA__;
...
# 单根线段大趋势分析
# =====================================================================
def analyze_segments_trend(fsegs, trend_pct=3.0, trend_bars=20):
    """
    对每根已完成线段单独分析，根据涨幅和持续时间判断是否为大趋势线段。
    不合并、不找盘整，纯单根判断。

    分类：
      - 大趋势上涨：方向向上 且 涨幅 >= trend_pct% 且 持续K线 >= trend_bars
      - 大趋势下跌：方向向下 且 跌幅 >= trend_pct% 且 持续K线 >= trend_bars
      - 普通波动：不满足上述条件

    返回：列表，每根线段一个 dict，含 idx/direction/change_pct/bar_count/
          slope/is_trend/trend_dir/reason。
    """
    results = []
    for i, seg in enumerate(fsegs, 1):
        base = seg.fx_a.fx
        change_pct = (seg.fx_b.fx - base) / base * 100 if base != 0 else 0
        bar_count = sum(len(b.bars) for b in seg.bis)
        slope = change_pct / bar_count if bar_count > 0 else 0  # 每根K线平均涨幅%
        ...
        results.append({
            ...
            "trend_dir": trend_dir,
            "reason": reason,
            "start_time": seg.fx_a.dt,
            "end_time": seg.fx_b.dt,
            "start_price": seg.fx_a.fx,
            "end_price": seg.fx_b.fx,
        })
    return results
...
def print_terminal(label, freq, sdt, edt, bars, prewarm, c):
    ...
    # 单根线段大趋势分析
    trends = analyze_segments_trend(fsegs)
    ...
    print("  阈值：涨幅>=3% 且 持续>=20根K线 → 大趋势线段")
    for t in trends:
        if t["is_trend"]:
...
# HTML 图表报告
# =====================================================================
def render_html(label, freq, sdt, edt, bars, prewarm, c, out_path=None):
    n = len(bars)
    dts = [b.dt for b in bars]
...
def main():
    ...
    try:
        ...
        print_terminal(label, freq, sdt, edt, bars, prewarm, c)

        want_html = True
        if not auto_mode:
            ans = input("是否生成 HTML 图表报告？(y/n，默认 y)：").strip().lower()
